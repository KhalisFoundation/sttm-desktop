 ...
 */
export const query = async (searchQuery, searchType, searchSource) => {
  if (!initialized) {
    init();
  }
  let dbQuery = '';
...
export const loadVerses = async (start, end) =>
...
export const loadCeremonies = async () => {
  ...
  try {
    const rows = await db.all('SELECT * FROM Ceremonies ORDER BY ID');
    return rows;
  } catch (e) {
    return e;
  }
...
export const getAng = async ShabadID => {
  ...
  try {
    ...
    const row = await db.get(q, [ShabadID]);
    return {
      PageNo: row.PageNo,
      SourceID: row.SourceID,
    };
...
export const getShabad = async VerseID => {
  ...
  try {
    const row = await db.get('SELECT ShabadID FROM Shabad WHERE VerseID = ?', [VerseID]);
    return row.ShabadID;
  } catch (e) {
    return e;
  }
