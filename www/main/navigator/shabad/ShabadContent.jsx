  ...
  const updateTraversedVerse = (newTraversedVerse, verseIndex, crossPlatformId = null) => {
    ...
    if (activeVerseId !== newTraversedVerse) {
      ...
      if (isSubtitleRecording) {
        ...
        subtitleData.push({
          seconds: Math.abs(currentTimestamp - timestamp)/1000,
          content: activeShabad[verseIndex].Gurmukhi,
        });
    ...
    if (isSundarGutkaBani && sundarGutkaBaniId) {
      ...
      loadShabad(activeShabadId, initialVerseId).then((verses) => {
        if (verses) {
          ...
          if (initialVerseId) {
            if (isSubtitleRecording) {
              const mappedShabadArray = filterRequiredVerseItems(verses);
              const currentIndex = mappedShabadArray.findIndex((obj) => obj.verseId === initialVerseId);
              const currentTimestamp = new Date();
              subtitleData.push({
                seconds: Math.abs(currentTimestamp - timestamp)/1000,
                content: mappedShabadArray[currentIndex].verse,
              });
              setTimestamp(currentTimestamp);
            }
